...
'Family issues':["“Life can be much broader once you discover one simple fact: Everything around you that you call life was made up by people that were no smarter than you. And you can change it, you can influence it… Once you learn that, you'll never be the same again.” —Steve Jobs",
...
options= "{'options':['Completely agree','Somewhat agree','Neutral','Somewhat disagree','Completely disagree']}"
questions = { 'Depression':["All the tasks you have performed, are taking much more time than usual.",
...
], 'Family Issues': ['Considerate of other people’s feelings',
...
['Sports', 'Yoga & Relaxation', 'Literature', 'Custom Hobbies']
]
 #Consider adding the add custom option
#gets activated once, Custom Hobbies is activated.



questions = { 'Progress':
[
"Are you following the treatment?",
"Are you feeling better?",
"Do you enjoy practicing the weekly activities?", 
"On a scale of 5, how satisfied are you with the treatment?"],
'Additional': 
[
"You feel, your life is sad, as there is no joy in your life anymore.",
"Feeling afraid, as if something awful might happen.",
"You are having trust issues with everyone around you.",
"What kind of weekly activities are you attracted to? ",
...
def initiate_all():
    for key, value in quotes.items():
        for q in value:
            l = Quote(q_class=key, text = q)
            l.save()


def initiate_all_q():
    #cat, question, options
    curr_options = []
    i = 0
    for key, value in questions.items():
        for q in value:
            ...
            is_slider = 0
            is_paragraph = 0
            is_multiple_choice, is_slider, is_paragraph, curr_options = (lambda x: (0, 0, 0, options[0]) if (x <3 )else ((0, 1, 0, []) if (x == 3) else((0, 0, 0, options[1]) if (x < 7) else ((1, 0, 0, options[-1]) if (x == 7) else (0, 0, 1, [])))))(i)
            faQ = Question(question = q, cat = key, options = curr_options, is_slider = is_slider, is_multiple_choice = is_multiple_choice, is_paragraph = is_paragraph)
            faQ.save()
            i += 1
